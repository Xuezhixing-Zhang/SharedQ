...
The saved post-search score is `sum(abs(theta(gamma) - theta_target))`. The optimizer is derivative-free COBYLA (`NLOPT_LN_COBYLA`) with random starts from `runif(gamma_length, -1, 1)`. Raw `gamma` has no finite lower or upper box bounds in the current scripts: `lb = -Inf`, `ub = Inf`. Any constraints below are inequality constraints on the projected Q-coefficients, using the `nloptr` convention `g(x) <= 0`. Saved artifacts may still be approximate if the bounded search stops before exact feasibility or exact target matching.

| Setting | Calibration constraints used |
| --- | --- |
| Setting I | Constrained projected Q-coefficients to practical windows: `Q3_A1` and `Q2_A1` in `[0.08, 0.32]`; `Q1_A1` in `[-0.30, -0.10]`; `Q3_A1A2` in `[0.25, 0.55]`; `Q3_A2` in `[0.05, 0.35]`; `Q3_A3` and `Q2_A2` in `[-0.75, -0.45]`; `Q3_A1A3` and `Q2_A1A2` in `[0.60, 1.00]`. No equality constraints are active. |
| Setting II | No active equality or inequality constraints on projected Q-coefficients. Calibration is driven by the objective only. |
| Setting III | Constrained selected projected Q-coefficients: `Q3_A3` in `[-0.75, -0.55]`; `Q3_O3:A3` in `[0.50, 0.70]`; `Q2_A2` in `[-0.40, -0.20]`; `Q1_A1` in `[0.00, 0.20]`; `Q1_O1:A1` in `[0.50, 0.70]`. No equality constraints are active. `tol_constraints_ineq = 1e-3` is supplied for these ten inequalities. |
| Supplementary Setting III No Shared | No active equality or inequality constraints on projected Q-coefficients. Calibration is driven by the separated-target objective only. |
