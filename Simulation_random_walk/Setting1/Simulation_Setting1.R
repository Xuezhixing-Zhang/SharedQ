...
setting1_simulation_results_dir <- file.path(setting1_dir, "simulation_results")
dir.create(setting1_calibration_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(setting1_simulation_results_dir, showWarnings = FALSE, recursive = TRUE)

source(file.path(setting1_dir, "Q_functions.R"))

run_setting1_simulation <- function(
  ...
  Q_learning_Setting_1(gamma_true, save = TRUE, mc_n = mc_n)
  ...
  for (n in ns) {
    ...
    for (i in seq_len(n_reps)) {
      print(paste("Setting I, n =", n, "rep =", i))
      results_n[[i]] <- Simu_I(
        n,
        gamma_true,
        theta_true,
      ...
      saveRDS(results_n, file.path(setting1_simulation_results_dir, paste0("results_", n, ".rds")))
      pb$tick()
    }
...
run_setting1_smoke_test <- function(
  n = 30,
  n_reps = 1,
  nfolds = 2,
  metric = "MSE_Q",
