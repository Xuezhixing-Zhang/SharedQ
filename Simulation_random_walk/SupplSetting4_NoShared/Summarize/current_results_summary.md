...
The accepted production calibration artifact is `calibration_pqff_separated_parsimonious.rds`. Accepted true estimates are the saved population-projected `theta` values after target-to-truth calibration/projection; do not report the scripted target vector alone.
