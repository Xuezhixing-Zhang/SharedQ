################## Shared Q ##############
SharedQ <- function(data_original, warmstart, max_iter = 5000, tol = 1e-06){
  ...
  while (is.finite(delta) && delta > tol && iter < max_iter) {
    ...
    if (any(bad)) {
      ...
      warning("SharedQ produced non-finite coefficients; replaced them with previous-iteration values.")
    }
    theta <- theta_new
    delta <- sqrt(sum((theta - theta_prev)^2))
