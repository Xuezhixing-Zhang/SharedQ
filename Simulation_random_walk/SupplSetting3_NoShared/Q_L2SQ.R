...
SQlearning_L2 <- function(data_original, 
  ...
  while (is.finite(delta) && delta > tol && iter < max_iter) {
    ...
    if (any(bad)) {
      ...
      warning("SQlearning_L2 produced non-finite coefficients; replaced them with previous-iteration values.")
    }
    theta <- theta_new
    delta <- sqrt(sum((theta - theta_prev)^2))
  }
  ...
  if (iter >= max_iter) { 
    warning("SQlearning_L2 reached max_iter before convergence.") 
    notes <- paste0("SQlearning_L2 reached max_iter before convergence.")
  }
