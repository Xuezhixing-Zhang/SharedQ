...
- [x] SET-I-REV 1: Replace Setting I `balanced_small` high-precision shared means/sigmas with rounded, defensible values and reduce the `Q3_A1` / `Q2_A1` near-shared target gap from `0.0600` to a clearly near-shared gap, preferably at most `0.02`.
- [ ] SET-I-REV 2: Recalibrate and validate Setting I under the revised `balanced_small` spec, promoting only an artifact whose stored target and calibrated true values satisfy the revised near-shared design.
- [ ] SET-I-REV 3: Rerun Setting I production simulations and regenerate summaries/manuscript tables after the revised Setting I calibration passes.
...
- Current production calibration status: Setting I `balanced_small` has been revised to a rounded random-shared target and is pending recalibration. The current Setting I production calibration artifact predates this target and must not be used for final reporting. Setting III `rs_tol006` remains the promoted default. Setting II and Supplementary Setting III No Shared keep their separated no-shared target definitions.
- Current Setting I review status: the earlier `balanced_small` validation and production outputs were internally consistent, but the `Q3_A1` / `Q2_A1` target gap was `0.0600` and the calibrated gap was `0.0668`, which was too large for a clean near-shared default. Those outputs are superseded by the rounded-target revision.
- Current Setting I revision status: `SET-I-REV 1` is complete. The rounded `balanced_small` replacement keeps seed `63`, uses `shared_mu=(0.13,-0.35,0.61)` and `shared_sigma=(0.006,0.00,0.006)`, and generates target gaps `0.019184`, `0.000000`, and `0.014354`. `SET-I-REV 2` is active: run isolated gate candidates, validate, and promote only a passing revised calibration artifact.
- Current Setting I calibration jobs: three isolated gate candidates were submitted on 2026-06-02 and are running in the `long` queue: `s1_rev_tol006` job `525279.hn-10-03`, `s1_rev_tol008` job `525281.hn-10-03`, and `s1_rev_tol009` job `525280.hn-10-03`. Each uses `CALIBRATION_MC_N=1000000`, `CALIBRATION_MAXEVAL=150000`, `CALIBRATION_LOCAL_MAXEVAL=30000`, and `CALIBRATION_N_STARTS=5`.
- Current bounded smoke status: older low-budget Setting I smoke reports are stale relative to the accepted 2026-05-28 target representation. After the 2026-06-02 rounded seed-63 Setting I revision, the old default Setting I calibration report fails target-definition validation as expected and is superseded. The active Setting I gate candidates are running in PBS; Setting III default validation and gate selection still pass after `rs_tol006` promotion.
...
- Current production result status: all four active random-walk production simulations submitted on 2026-05-28 completed by 2026-05-29. Setting I `523877.hn-10-03`, Setting II `523878.hn-10-03`, Setting III `523879.hn-10-03`, and Supplementary Setting III No Shared `523880.hn-10-03` each produced complete `200/200` replicate outputs for `n = 100, 300, 500, 1000`, but the Setting I outputs predate the rounded-target revision and are no longer final for Setting I reporting. Full evaluation summaries and manuscript-style tables were regenerated under `Simulation_random_walk/Writing/generated_tables/`.
...
- Current Setting IV production status: synthetic-parametric Setting IV and Supplementary Setting IV No Shared completed `200/200` repetitions for `n = 100, 300, 500, 1000` on 2026-06-02 after force-regenerating the target-to-truth calibration/projection artifacts. Setting IV and Supplementary Setting IV No Shared default calibration validation pass; the full validator exits nonzero only because revised Setting I calibration is still pending.
- Current Setting IV redo status: user clarified on 2026-06-02 that Setting IV must explicitly follow the same target-to-truth workflow to obtain true estimates. Setting IV and Supplementary Setting IV No Shared were force-recalibrated from target to projected truth and rerun; accepted true estimates are the calibration artifact's saved projected `theta`, not the target vector alone.
