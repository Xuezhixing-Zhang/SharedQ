...
- Next real work: run smoke checks for the corrected target builders, submit replacement gate-candidate calibration jobs for Setting I and Setting III, validate and promote only passing corrected artifacts, then rerun full simulations for all settings and regenerate summaries/tables. The evaluation phase should still check whether the proposed method beats old SharedQ and conventional Q in most settings before writing the manuscript-facing tables.
