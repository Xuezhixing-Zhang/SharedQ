...
- Setting IV setup status: `PROJECT_QUIT_FOREVER_FREE_SETTING_IV_DESIGN.md` is the root design note for the Project Quit / Forever Free parsimonious two-stage design. Implementation folders have been initialized at `Simulation_random_walk/Setting4/` and `Simulation_random_walk/SupplSetting4_NoShared/`, with initial README, current-results, todo, and artifact subfolders. Calibration and simulation code are pending.
...
- Setting IV next work: after the active recalibration jobs are under control, implement the two-stage Project Quit / Forever Free pipeline from the todo files. Setting IV must not be included in production tables until default calibration artifacts exist and pass validation.
