...
- Current production calibration status: not accepted. Existing accepted production `.rds` coverage is incomplete for the new constraints, so production simulations remain blocked until the default validation gate passes for all four settings.
- Current bounded smoke status: partly accepted for default specs only. Setting II `separated_moderate`, Setting III `rw_sigma_moderate`, and Supplementary Setting III No Shared `separated_moderate` pass their per-setting smoke gates after larger-budget default calibration. Setting I original `balanced_small` failed the stricter `0.01` gate after higher-budget retries, so its default target was tuned to `shared_mu=(0.12,-0.35,0.61)` and `shared_sigma=(0.03,0.00,0.01)` after low-cost feasibility probes.
- Current scheduler checkpoint: Setting II `509057.hn-10-03` and Setting III `509058.hn-10-03` are still running from the 2026-05-24 default production calibration batch. Supplementary Setting III No Shared `509059.hn-10-03` finished and passed default validation. Setting I `509056.hn-10-03` and its higher-effort retry `510190.hn-10-03` failed materially, so the default target was revised and resubmitted as `511306.hn-10-03`. Additional isolated gate candidates are running for Setting I (`511381`, `511383`), Setting II (`511382`, `511380`), Setting III (`511389`, `511391`), and Supplementary No Shared (`511390`, `511392`).
...
- Next real work: monitor Setting I `511306.hn-10-03`, Setting II `509057.hn-10-03`, Setting III `509058.hn-10-03`, and the new isolated gate-candidate jobs; run `Rscript Simulation_random_walk/select_calibration_gate_candidate.R` after gate candidates finish; then run `VALIDATION_SPEC_MODE=default Rscript Simulation_random_walk/validate_candidate_calibration.R` after accepted artifacts are promoted; only then submit full production simulations/evaluations.
