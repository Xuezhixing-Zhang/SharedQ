...
- [ ] SET-I-REV 1: Replace Setting I `balanced_small` high-precision shared means/sigmas with rounded, defensible values and reduce the `Q3_A1` / `Q2_A1` near-shared target gap from `0.0600` to a clearly near-shared gap, preferably at most `0.02`.
...
- Current Setting I review status: the existing `balanced_small` validation and production outputs are internally consistent, but the `Q3_A1` / `Q2_A1` target gap is `0.0600` and the calibrated gap is `0.0668`, which is too large for a clean near-shared default. The high-precision Setting I shared means/sigmas should be replaced with rounded values before final reporting, followed by Setting I-only recalibration, validation, production rerun, and table regeneration.
