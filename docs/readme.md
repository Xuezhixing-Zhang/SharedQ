...
For Project Quit / Forever Free Setting IV work, read the Setting IV sections of `RANDOM_WALK_SETTING_DESIGN_SUMMARY.md`, then use `Simulation_random_walk/Setting4/` and `Simulation_random_walk/SupplSetting4_NoShared/` as the implementation folders. Setting IV shared true parameters must also follow the random shared rule in this README.

Setting IV is a synthetic-parametric simulation design. The cleaned PQ/FF data at `Simulation_random_walk/Setting4/source_data/cleaned_data.05.21.csv` is used only to understand structure and set aggregate design constants; production simulations must not resample real participant histories or outcomes. Setting IV and Supplementary Setting IV No Shared still must obtain accepted true estimates through the same target-to-truth workflow as the other settings: define `theta_target`, construct/calibrate outcome-generating `gamma`, generate a large synthetic Monte Carlo population, project the working Q-model coefficients, save the projected `theta` in the calibration artifact, validate the artifact, then run production simulations from that artifact. Do not report scripted target values as true estimates unless they have passed through this calibration/projection artifact workflow. The relevant todo files are:
...
Setting IV production outputs are accepted only when generated by the synthetic-parametric DGP, stored as unsuffixed `results_<n>.rds`, and backed by `source_mode = synthetic_parametric` calibration artifacts whose saved projected `theta` has passed candidate validation. Do not use real-source, source-data-resampling, or target-only artifacts in manuscript tables or method claims.
