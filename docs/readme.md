...
Setting IV production outputs are accepted only when generated by the synthetic-parametric DGP, stored as unsuffixed `results_<n>.rds`, and backed by `source_mode = synthetic_parametric` calibration artifacts. Do not use real-source or source-data-resampling artifacts in manuscript tables or method claims.
